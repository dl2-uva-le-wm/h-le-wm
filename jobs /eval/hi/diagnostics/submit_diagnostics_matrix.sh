...
JOB_SCRIPT="${JOB_SCRIPT:-${SCRIPT_DIR}/run_diagnostics_matrix.sh}"
...
for i in "${!CHECKPOINT_ROWS[@]}"; do
  ...
  sbatch \
    --array="1-${NUM_CONFIGS}" \
    --output="${LOG_DIR}/run_diagnostics_matrix_%A_%a.out" \
    --error="${LOG_DIR}/run_diagnostics_matrix_%A_%a.err" \
