...
# Wrapper job: keep the current best completed low-level setup and replan the
# high-level subgoal more often (k=3 instead of 5).
#
# Suggested use:
#   cd jobs/eval/hi
#   sbatch eval_hope1_short_hier_soft_lowh2_blk5_highreplan3.sh

#SBATCH --partition=rome
#SBATCH --job-name=hi_eval_hope1_short_hier_soft_lowh2_blk5_highreplan3
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=02:00:00
#SBATCH --output=eval_hope1_short_hier_soft_lowh2_blk5_highreplan3_%j.out
#SBATCH --error=eval_hope1_short_hier_soft_lowh2_blk5_highreplan3_%j.err
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_hier_soft_lowh2_blk5_highreplan3_d${GOAL_OFFSET_STEPS}_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
