...
# Hope2 PushT eval on d=50 with the same d50 CEM settings as the paired d50
# run, but using high-level horizon H=2.

#SBATCH --partition=rome
#SBATCH --job-name=hi_eval_hope2_d50_h2
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=04:00:00
#SBATCH --output=hope2_d50_h2_eval_%j.out
#SBATCH --error=hope2_d50_h2_eval_%j.err
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_hope2_d50_h2_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
