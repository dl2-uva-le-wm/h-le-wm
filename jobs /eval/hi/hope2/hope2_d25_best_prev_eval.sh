...
# Hope2 PushT eval on d=25 using the best hope1 planning setup observed so far.
# This keeps the short high-level horizon and low-level horizon=2 configuration.

#SBATCH --partition=rome
#SBATCH --job-name=hi_eval_hope2_d25_bestprev
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=02:30:00
#SBATCH --output=hope2_d25_best_prev_eval_%j.out
#SBATCH --error=hope2_d25_best_prev_eval_%j.err
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_hope2_d25_bestprev_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
