...
# Hope2 PushT eval on d=50 with the same higher-compute paper-scaled CEM budget
# as the paired d50 script, but with high-level horizon H=2.

#SBATCH --partition=rome
#SBATCH --job-name=hi_eval_hope2_d50_pscaled_h2
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=05:00:00
#SBATCH --output=hope2_d50_paperscaled_h2_eval_%j.out
#SBATCH --error=hope2_d50_paperscaled_h2_eval_%j.err
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_hope2_d50_paperscaled_h2_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
