...
# Hope2 PushT eval on d=25 with the same d25 CEM settings as the best prior
# setup, but using high-level horizon H=2.

#SBATCH --partition=rome
#SBATCH --job-name=hi_eval_hope2_d25_h2
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=02:30:00
#SBATCH --output=hope2_d25_h2_eval_%j.out
#SBATCH --error=hope2_d25_h2_eval_%j.err
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_hope2_d25_h2_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
