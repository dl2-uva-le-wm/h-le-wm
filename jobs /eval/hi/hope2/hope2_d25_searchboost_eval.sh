...
# Hope2 PushT eval on d=25 with the best prior short-horizon setup, but with a
# stronger low-level CEM search.

#SBATCH --partition=rome
#SBATCH --job-name=hi_eval_hope2_d25_searchboost
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=03:00:00
#SBATCH --output=hope2_d25_searchboost_eval_%j.out
#SBATCH --error=hope2_d25_searchboost_eval_%j.err
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_hope2_d25_searchboost_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
