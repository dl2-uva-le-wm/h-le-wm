...
# PushT eval for the completed latent-dim-8 fixed-stride-5 N=4 run, pinned to
# its final object checkpoint (epoch 15), using the standard d=50 H=2 GPU setup.

#SBATCH --partition=gpu_a100
#SBATCH --gpus=1
#SBATCH --job-name=hi_eval_s5n4e15_d50
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=04:00:00
#SBATCH --output=stride5n4_ep15_d50_eval_%j.out
#SBATCH --error=stride5n4_ep15_d50_eval_%j.err
...
export RUN_NAME="${RUN_NAME:-hi_lewm_p2_train_latent_action_dim_8_stride_5_n4_22518175}"
export CHECKPOINT_EPOCH="${CHECKPOINT_EPOCH:-15}"
export MODEL_LABEL="${MODEL_LABEL:-stride5n4_ep15}"
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_${MODEL_LABEL}_d50_h2_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
export RESULT_FILENAME="${RESULT_FILENAME:-${MODEL_LABEL}_d50_h2_results.txt}"
