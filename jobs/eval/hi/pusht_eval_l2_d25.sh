...
# Usage:
#   cd jobs/eval/hi
#   sbatch pusht_eval_l2_d25.sh
#
# Optional overrides:
#   sbatch --export=ALL,POLICY=hi_lewm_l2_d25_epoch_1 pusht_eval_l2_d25.sh
#   sbatch --export=ALL,STABLEWM_HOME=/scratch-shared/$USER/stablewm_data pusht_eval_l2_d25.sh
...
#SBATCH --output=out/hi_pusht_eval_%j.out
#SBATCH --error=out/hi_pusht_eval_%j.err
...
if [[ ! -f "${CKPT_OBJECT_PATH}" ]]; then
  ...
  echo "  sbatch --export=ALL,POLICY=hi_lewm_l2_d25_epoch_1 pusht_eval_l2_d25.sh" >&2
  exit 4
fi
