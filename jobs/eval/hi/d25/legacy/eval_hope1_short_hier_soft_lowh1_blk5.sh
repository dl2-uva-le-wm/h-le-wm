...
# Wrapper job: short hierarchical-soft eval with an even shorter low-level
# horizon, keeping the original action_block=5.
#
# Suggested use:
#   cd jobs/eval/hi
#   sbatch eval_hope1_short_hier_soft_lowh1_blk5.sh

#SBATCH --partition=rome
#SBATCH --job-name=hi_eval_hope1_short_hier_soft_lowh1_blk5
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=02:00:00
#SBATCH --output=eval_hope1_short_hier_soft_lowh1_blk5_%j.out
#SBATCH --error=eval_hope1_short_hier_soft_lowh1_blk5_%j.err
...
export LOW_HORIZON="${LOW_HORIZON:-1}"
export LOW_RECEDING_HORIZON="${LOW_RECEDING_HORIZON:-1}"
export LOW_ACTION_BLOCK="${LOW_ACTION_BLOCK:-5}"

export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_hier_soft_lowh1_blk5_d${GOAL_OFFSET_STEPS}_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
