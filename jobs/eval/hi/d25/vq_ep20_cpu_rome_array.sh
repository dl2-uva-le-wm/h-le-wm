...
#
# Sweep design:
# - Focus on d=25, budget=50 because prior PushT hierarchical results showed
#   this regime was informative and stable.
# - Keep LOW_ACTION_BLOCK=5 fixed because this checkpoint family expects grouped
#   low-level actions with that width.
# - Center the sweep on the strongest old regime: LOW_HORIZON=2.
# - Vary only the knobs that were previously most sensitive:
#   low-level topk / samples, high-level topk, high-level replanning cadence,
#   and one LOW_HORIZON=3 control.
#
# Usage:
#   cd jobs/eval/hi/d25
...
#
# Optional overrides:
#   sbatch --array=0-3 vq_ep20_cpu_rome_array.sh
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_${MODEL_LABEL}_d25_b${EVAL_BUDGET}_job_${SLURM_ARRAY_JOB_ID:-manual}_${TASK_ID}}"
