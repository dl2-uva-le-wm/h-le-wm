...
# Wrapper job: start from the more reactive lowh2/blk3 setup and reinvest the
# cheaper low-level solve into a stronger low-level search.
#
# Suggested use:
#   cd jobs/eval/hi
...
#SBATCH --job-name=hi_eval_hope1_short_hier_soft_lowh2_blk3_searchboost
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=03:00:00
#SBATCH --output=eval_hope1_short_hier_soft_lowh2_blk3_searchboost_%j.out
#SBATCH --error=eval_hope1_short_hier_soft_lowh2_blk3_searchboost_%j.err
...
export EVAL_SUBDIR="${EVAL_SUBDIR:-eval_hier_soft_lowh2_blk3_searchboost_d${GOAL_OFFSET_STEPS}_b${EVAL_BUDGET}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
