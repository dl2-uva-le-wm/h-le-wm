...
These are the practical knobs exposed by the job scripts in `jobs/eval/hi/d25/` and `jobs/eval/hi/d50/`.
...
These fields live in `config/eval/hi_pusht.yaml`. Some are exposed by job scripts, some are only overridden through Hydra.
