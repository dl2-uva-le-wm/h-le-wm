...
#SBATCH --output=out/download_datasets_%j.out
#SBATCH --error=out/download_datasets_%j.err
...
exec > >(tee -a "${LOG_DIR}/download_datasets_${JOB_TAG}.out") \
     2> >(tee -a "${LOG_DIR}/download_datasets_${JOB_TAG}.err" >&2)
