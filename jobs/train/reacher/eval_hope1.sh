...
# Snellius eval job (Reacher):
# - Evaluates the latest (or selected) checkpoint from a Reacher hope1 training run
# - Uses hi_eval.py with the hi_reacher config
# - Runs on the rome CPU partition, consistent with the CPU eval flow used for PushT
#
# Usage:
...
#
# Common overrides:
#   sbatch --export=ALL,RUN_NAME=hi_lewm_reacher_train_hope1_22517097 eval_hope1.sh
#   sbatch --export=ALL,CHECKPOINT_EPOCH=10 eval_hope1.sh
#   sbatch --export=ALL,GOAL_OFFSET_STEPS=25,EVAL_BUDGET=50 eval_hope1.sh
#   sbatch --export=ALL,STABLEWM_HOME=/scratch-shared/$USER/stablewm_data eval_hope1.sh

#SBATCH --partition=rome
#SBATCH --gpus=0
#SBATCH --job-name=hi_eval_reacher_hope1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=03:00:00
#SBATCH --output=eval_reacher_hope1_%j.out
#SBATCH --error=eval_reacher_hope1_%j.err
  ...
  for c in \
    ...
    for p in "${c}" "${c}/.." "${c}/../.." "${c}/../../.."; do
      if p="$(cd "${p}" >/dev/null 2>&1 && pwd)"; then
        if [[ -f "${p}/hi_eval.py" && -f "${p}/config/eval/hi_reacher.yaml" ]]; then
          echo "${p}"
          return 0
        fi
      fi
...
resolve_default_run_name() {
  local runs_root="$1"
  shopt -s nullglob
  local candidates=( "${runs_root}"/hi_lewm_reacher_train_hope1_* )
  shopt -u nullglob
  ...
  echo "${sorted_runs[${#sorted_runs[@]}-1]}"
}
...
# Defaults mirror config/eval/hi_reacher.yaml unless overridden.
HIGH_NUM_SAMPLES="${HIGH_NUM_SAMPLES:-300}"
...
RUNS_ROOT="${STABLEWM_HOME}/runs"
RUN_NAME="${RUN_NAME:-}"
if [[ -z "${RUN_NAME}" ]]; then
  if ! RUN_NAME="$(resolve_default_run_name "${RUNS_ROOT}")"; then
    echo "ERROR: Could not auto-detect a Reacher hope1 run under ${RUNS_ROOT}" >&2
    echo "Pass RUN_NAME explicitly, for example:" >&2
    echo "  sbatch --export=ALL,RUN_NAME=hi_lewm_reacher_train_hope1_<jobid> eval_hope1.sh" >&2
    exit 3
  fi
fi

RUN_DIR="${RUNS_ROOT}/${RUN_NAME}"
DATASET_PATH="${STABLEWM_HOME}/${DATASET_FILE}"

if [[ ! -f "${DATASET_PATH}" ]]; then
  echo "ERROR: dataset file not found: ${DATASET_PATH}" >&2
  echo "Run setup first, for example:" >&2
  echo "  sbatch --export=ALL,STABLEWM_HOME=${STABLEWM_HOME} jobs/setup/download_reacher.sh" >&2
  exit 4
fi
...
EVAL_SUBDIR="${EVAL_SUBDIR:-eval_reacher_d${GOAL_OFFSET_STEPS}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
RESULT_FILENAME="${RESULT_FILENAME:-${POLICY_BASENAME}_hi_reacher_results_d${GOAL_OFFSET_STEPS}.txt}"
