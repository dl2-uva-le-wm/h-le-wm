...
# Snellius eval job (OGBench-Cube):
# - Evaluates the latest (or selected) checkpoint from an OGBench-Cube hope1 training run
# - Uses hi_eval.py with the hi_cube config
# - Runs on the rome CPU partition, consistent with the CPU eval flow used for PushT and Reacher
#
# Usage:
#   cd jobs/train/cube
...
#
# Common overrides:
#   sbatch --export=ALL,RUN_NAME=hi_lewm_cube_train_hope1_22517097 eval_hope1.sh
...
#SBATCH --gpus=0
#SBATCH --job-name=hi_eval_cube_hope1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=03:00:00
#SBATCH --output=eval_cube_hope1_%j.out
#SBATCH --error=eval_cube_hope1_%j.err
...
resolve_default_run_name() {
  local runs_root="$1"
  shopt -s nullglob
  local candidates=( "${runs_root}"/hi_lewm_cube_train_hope1_* )
  shopt -u nullglob
...
DATASET_FILE="${DATASET_FILE:-ogbench/cube_single_expert.h5}"
DATASET_NAME="${DATASET_NAME:-ogbench/cube_single_expert}"
CONFIG_NAME="${CONFIG_NAME:-hi_cube}"
...
# Defaults mirror config/eval/hi_cube.yaml unless overridden.
...
if [[ -z "${RUN_NAME}" ]]; then
  if ! RUN_NAME="$(resolve_default_run_name "${RUNS_ROOT}")"; then
    echo "ERROR: Could not auto-detect an OGBench-Cube hope1 run under ${RUNS_ROOT}" >&2
    echo "Pass RUN_NAME explicitly, for example:" >&2
    echo "  sbatch --export=ALL,RUN_NAME=hi_lewm_cube_train_hope1_<jobid> eval_hope1.sh" >&2
    exit 3
  fi
fi
...
EVAL_SUBDIR="${EVAL_SUBDIR:-eval_cube_d${GOAL_OFFSET_STEPS}_job_${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
RESULT_FILENAME="${RESULT_FILENAME:-${POLICY_BASENAME}_hi_cube_results_d${GOAL_OFFSET_STEPS}.txt}"
