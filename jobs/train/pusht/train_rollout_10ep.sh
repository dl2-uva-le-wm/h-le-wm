...
# Snellius job: Train hierarchical PushT P2 with rollout loss enabled (10 epochs).
# Usage (from this folder):
#   cd jobs/train/pusht
#   sbatch train_rollout_10ep.sh
#
# Optional overrides:
...
if [[ -z "${WANDB_API_KEY:-}" ]]; then
  ...
  echo "Set it in ${WANDB_ENV_FILE} or submit with: sbatch --export=ALL,WANDB_API_KEY=<your_key> train_rollout_10ep.sh" >&2
  exit 2
fi
...
CMD=(
  ...
  loss.alpha=0.0
  loss.beta=1.0
  loss.rollout.enabled=true
  loss.rollout.weight="${ROLLOUT_WEIGHT}"
  loss.rollout.steps="${ROLLOUT_STEPS}"
)
