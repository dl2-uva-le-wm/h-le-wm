...
# Snellius training job: PushT high-level P2 with VQ macro-action encoder.
# - Reads dataset + pretrained low-level checkpoint from node-local TMPDIR.
# - Saves training artifacts directly to shared scratch.
# - Keeps the pretrained low-level stack frozen.
#
# Usage:
#   cd jobs/train/pusht
#   sbatch train_vq_hope1.sh
#
# Optional overrides:
#   MAX_EPOCHS=10 sbatch train_vq_hope1.sh
#   TRAIN_RUN_NAME=hi_lewm_p2_vq_train_custom sbatch train_vq_hope1.sh
#   LATENT_ACTION_DIM=16 VQ_NUM_CODES=128 sbatch train_vq_hope1.sh
#   SCRATCH_STABLEWM_HOME=/scratch-shared/$USER/stablewm_data sbatch train_vq_hope1.sh

#SBATCH --partition=gpu_a100
#SBATCH --constraint=scratch-node
#SBATCH --gpus=1
#SBATCH --job-name=hi_l2_pusht_vq_hope1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=9
#SBATCH --time=07:00:00
#SBATCH --output=train_vq_hope1_%j.out
#SBATCH --error=train_vq_hope1_%j.err
...
if [[ -z "${WANDB_API_KEY:-}" ]]; then
  ...
  echo "Set it in ${WANDB_ENV_FILE} or submit with: sbatch --export=ALL,WANDB_API_KEY=<your_key> train_vq_hope1.sh" >&2
  exit 2
fi
...
TRAIN_RUN_NAME="${TRAIN_RUN_NAME:-hi_lewm_p2_vq_train_hope1_${SLURM_JOB_ID:-manual}}"
WANDB_RUN_ID="${WANDB_RUN_ID:-run_${SLURM_JOB_ID:-manual}}"

# VQ defaults are explicit here so the run does not depend on future config drift.
LATENT_ACTION_DIM="${LATENT_ACTION_DIM:-16}"
VQ_NUM_CODES="${VQ_NUM_CODES:-128}"
VQ_DECODER_HIDDEN_DIM="${VQ_DECODER_HIDDEN_DIM:-768}"
VQ_RECON_WEIGHT="${VQ_RECON_WEIGHT:-1.0}"
VQ_COMMITMENT_WEIGHT="${VQ_COMMITMENT_WEIGHT:-0.25}"
VQ_CODEBOOK_WEIGHT="${VQ_CODEBOOK_WEIGHT:-1.0}"
...
echo "Macro-action backend: VQ"
echo "Latent action dim: ${LATENT_ACTION_DIM}"
echo "VQ num codes: ${VQ_NUM_CODES}"
echo "VQ decoder hidden dim: ${VQ_DECODER_HIDDEN_DIM}"
echo "VQ loss weights: recon=${VQ_RECON_WEIGHT}, commitment=${VQ_COMMITMENT_WEIGHT}, codebook=${VQ_CODEBOOK_WEIGHT}"
echo "Low-level stack: pretrained + frozen"
...
CMD=(
  ...
  latent_action_encoder.vq.num_codes="${VQ_NUM_CODES}"
  latent_action_encoder.vq.decoder_hidden_dim="${VQ_DECODER_HIDDEN_DIM}"
  loss.alpha=0.0
  loss.beta=1.0
  loss.sigreg.weight=0.0
  loss.vq.recon_weight="${VQ_RECON_WEIGHT}"
  loss.vq.commitment_weight="${VQ_COMMITMENT_WEIGHT}"
  loss.vq.codebook_weight="${VQ_CODEBOOK_WEIGHT}"
)
